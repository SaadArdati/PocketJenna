...
class FirestoreQueryBuilder<T> extends StatefulWidget {
  ...
  final Future<List<T>> Function(int limit) query;
  ...
  final FirestoreQueryBuilderSnapshotBuilder<T> builder;
  ...
  _FirestoreQueryBuilderState<T> createState() =>
      _FirestoreQueryBuilderState<T>();
}

class _FirestoreQueryBuilderState<T> extends State<FirestoreQueryBuilder<T>> {
  var _pageCount = 0;

  late var _snapshot = _QueryBuilderSnapshot<T>._(
    docs: [],
    error: null,
    hasData: false,
    hasError: false,
  ...
  @override
  void didUpdateWidget(FirestoreQueryBuilder<T> oldWidget) {
  ...
  void _fetchQuery({bool nextPage = false}) {
    if (nextPage) {
...
class FirestoreListView<Document> extends FirestoreQueryBuilder<Document> {
  ...
  FirestoreListView({
    ...
    required FirestoreItemBuilder<dynamic> itemBuilder,
    super.pageSize,
